...
class FmCoreC1 extends Core {
	...
	public function getData($page) {
		...
		if (!empty($page['datas'])) {
			...
			if ($p[0]['params']['footer'] == 2) {
				...
				$menu = pdo_fetch('select * from ' . tablename('goddess_templates_designer_menu') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $menuid, ':uniacid' => $_W['uniacid']));
				if (!empty($menu)) {
	...
	public function gettagname($tagid,$tagpid,$tagtid, $rid) {
		...
		if ($tagid && $tagpid && $tagtid) {
			$tagt = pdo_fetch("SELECT title,parentid FROM ".tablename($this->table_tags)." WHERE id = :id AND rid = :rid ORDER BY id DESC", array(':id' => $tagtid, ':rid' => $rid));
	...
	public function GetPaihangcha($rid, $tfrom_user,$indexpx='') {
		 ...
		 * 判断赛区是否开启投票
		 */
		$userinfo = pdo_get('goddess_voteer',array('rid'=>$rid,'from_user'=>$tfrom_user));
		if(!pdo_get('goddess_game_area',array('rid'=>$rid,'id'=>$userinfo['area']))['isvote']){
			//是否开启单学校投票模式
			if(pdo_get('goddess_reply',array('rid'=>$rid))['isschool_mode']){
				//判断是否学校是否开启投票  学校排名
				if(!pdo_get('goddess_ly_school',array('rid'=>$rid,'id'=>$userinfo['school']))['isvote']){
					//开启
					$where .= " AND school = ".$userinfo['school'];
				}else{
					$where .=' AND school = -1';
				}
			}else{
				//没有开启单学校模式，整个赛区排名
				$where .= ' AND area = '.$userinfo['area'];
			}
		}else{
			//赛区未开启投票  不进行排名
			$where .= ' AND area = -1';
		...
		$ranks = pdo_fetchall('SELECT photosnum, xnphotosnum,from_user,hits,xnhits  FROM '.tablename($this->table_users).' WHERE status =1 '.$where.' AND rid = '.$rid.$order.'');
	...
	public function GetPaihangchaa($rid, $tfrom_user,$indexpx='') {
		global $_W;
		$date = array();
		$where = '';
		/**
		 * 判断赛区是否开启投票
		 */
		$userinfo = pdo_get('goddess_provevote',array('rid'=>$rid,'from_user'=>$tfrom_user));
		if(!pdo_get('goddess_game_area',array('rid'=>$rid,'id'=>$userinfo['area']))['isvote']){
			//是否开启单学校投票模式
			if(pdo_get('goddess_reply',array('rid'=>$rid))['isschool_mode']){
				//判断是否学校是否开启投票  学校排名
	...
	public function gettvotes($rid, $from_user, $indexpx){
		...
		foreach ($votes as $key => $value) {
			...
			$r[$key]['avatar'] = $this->getphotos($r[$key]['tfmimage']['photos'], $r[$key]['tuser']['avatar'], 'addons/goddess/static/mobile/public/images/no-avatar.png');
		...
		if ($type == 'avatar') {
			...
			if (empty($avatar)) {
				...
				if (empty($avatar)) {
					...
					$avatar = tomedia($username['avatar']);
					if (empty($avatar)) {
						$avatar = tomedia('./addons/goddess/icon.jpg');
					}
				}
			}
			return $avatar;
	...
	public function addjifen($rid, $from_user, $tfrom_user,$info = array(),$vote = array(),$type='vote') {
		...
		if ($rjifen['is_open_jifen']) {
			...
			if ($rjifen['is_open_jifen_sync']) {
				...
				if (!empty($uid)) {
					mc_credit_update($uid, 'credit1', $jifen, array(0, $msg,'goddess'));
				...
				if (!empty($tuid)) {
					mc_credit_update($tuid, 'credit1', $tjifen, array(0, $tmsg,'goddess'));
	...
	public function jsjifen($rid, $from_user, $jifen, $gifttitle, $type = 'gift') {
		...
		if ($rjifen) {
			...
			if (!empty($uid)) {
				...
				fm_mc_credit_update($uid, 'credit1', $jifen, 'js', array(0, $msg,'goddess'));
			}
		}
	...
	public function addjifencharge($rid, $from_user, $jifen, $ordersn) {
		...
		if ($rjifen) {
			...
			if (!empty($uid)) {
				...
				fm_mc_credit_update($uid, 'credit1', $jifen, 'add', array(0, $msg,'goddess'));
				...
				if (empty($voteer['jifen'])) {
					...
					if ($lastjifen > 0) {
						if ($lastjifen != $voteer['jifen'] && $voteer['jifen'] <= 0) {
							...
							fm_mc_credit_update($uid, 'credit1', $lastjifen, false, array(0, $msg,'goddess'));
						}
						$voteer = pdo_fetch("SELECT jifen FROM ".tablename($this->table_voteer)." WHERE from_user = :from_user and rid = :rid limit 1", array(':from_user' => $from_user,':rid' => $rid));
						...
						if (!empty($uid)) {
							...
							fm_mc_credit_update($uid, 'credit1', $voteer['jifen'], false, array(0, $msg,'goddess'));
							$lastjifen = $voteer['jifen'];
						}else{
							$lastjifen = $voteer['jifen'];
						}
	...
	public function editjifen($rid, $from_user, $jifen,$nickname,$avatar,$realname,$mobile,$sex) {
		...
		if ($rjifen) {
			...
			if (!empty($uid)) {
				...
				fm_mc_credit_update($uid, 'credit1', $jifen, false, array(0, $msg,'goddess'));
			}
		}
		$voteer_data = array(
			'uniacid' => $_W['uniacid'],
	...
	public function createvoteer($rid,$uniacid, $from_user,$nickname,$avatar,$sex) {
		if (!empty($from_user)) {
			...
			if (empty($voteer)) {
				...
				pdo_update($this->table_voteer, array('lasttime' => $time, 'ip' => getip(), 'iparr' => getiparr(getip())),array('from_user'=>$from_user,'rid'=>$rid));
	...
	public function votecode($rid, $from_user, $tfrom_user) {
		...
		$onlyoauth = pdo_fetch("SELECT fmauthtoken FROM ".tablename('goddess_onlyoauth')." WHERE 1 ORDER BY id DESC LIMIT 1");
	...
	public function getregusermoney($rid, $tfrom_user) {
		$total_gift = round(pdo_fetchcolumn("SELECT SUM(price) FROM ".tablename($this->table_order)." WHERE rid = :rid AND tfrom_user =:tfrom_user AND status = 1", array(':rid' => $rid, ':tfrom_user' => $tfrom_user)), 2);
		if (empty($total_gift)) {
			return '0';
		}else{
			return $total_gift;
