...
class FmCoreC2 extends FmCoreC1 {
	public function downloadqrcodeimg($ticket, $filename) {
		...
		$updir = '../attachment/images/goddess_qrcode/' . $uniacid . '/' . date("Y") . '/' . date("m") . '/';
		if (!is_dir($updir)) {
			mkdirs($updir);
		}
	...
	public function downloadurl2qr($url, $filename) {
		...
		$dfileurl = 'attachment/images/goddess_qrcode/' . $uniacid . '/' . date("Y") . '/' . date("m") . '/';
		$fileurl = '../' . $dfileurl;
		load() -> func('file');
		if (!is_dir($fileurl)) {
			mkdirs($fileurl);
	...
	public function doMobileQrcode() {
		...
		//print_r($ewmurl);
			...
			$save_dir = '../addons/goddess/haibao/' . $rid . '/' . date("Y") . '/' . date("m") . '/';
			if (!is_dir($save_dir)) {
				mkdirs($save_dir);
			}
