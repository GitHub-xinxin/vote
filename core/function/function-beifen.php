...
function update_tags_piaoshu ($rid) {
	$tags = pdo_fetchall("SELECT * FROM ".tablename('goddess_tags')." WHERE rid = :rid", array(':rid' => $rid));
	foreach ($tags as $key => $row) {
		if ($row['icon'] == 1) {
			$photosnum = pdo_fetchcolumn('SELECT SUM(photosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagpid = :tagpid AND status = 1', array(':rid' => $rid,':tagpid' => $row['id']));
			$xnphotosnum = pdo_fetchcolumn('SELECT SUM(xnphotosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagpid = :tagpid  AND status = 1', array(':rid' => $rid,':tagpid' => $row['id']));
			//pdo_update($this->table_tags, array('piaoshu' => $photosnum + $xnphotosnum), array('rid' => $rid, 'id' => $row['id']));
		}elseif ($row['icon'] == 2) {
			$photosnum = pdo_fetchcolumn('SELECT SUM(photosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagid = :tagid AND status = 1', array(':rid' => $rid,':tagid' => $row['id']));
			$xnphotosnum = pdo_fetchcolumn('SELECT SUM(xnphotosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagid = :tagid  AND status = 1', array(':rid' => $rid,':tagid' => $row['id']));
			//pdo_update($this->table_tags, array('piaoshu' => $photosnum + $xnphotosnum), array('rid' => $rid, 'id' => $row['id']));
		}elseif ($row['icon'] == 3) {
			$photosnum = pdo_fetchcolumn('SELECT SUM(photosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagtid = :tagtid AND status = 1', array(':rid' => $rid,':tagtid' => $row['id']));
			$xnphotosnum = pdo_fetchcolumn('SELECT SUM(xnphotosnum) FROM '.tablename('goddess_provevote').' WHERE rid = :rid AND tagtid = :tagtid  AND status = 1', array(':rid' => $rid,':tagtid' => $row['id']));
		}
		pdo_update('goddess_tags', array('piaoshu' => $photosnum + $xnphotosnum), array('rid' => $rid, 'id' => $row['id']));
	}
}
