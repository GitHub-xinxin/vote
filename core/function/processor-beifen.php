 ...
 */
	...
	function getphotos($avatar, $from_user, $rid, $is = '',$goddess_provevote_picarr) {
		$photo = getpicarrp($rid, $from_user,'1',$goddess_provevote_picarr);
		if ($is == 'avatar') {
			if (!empty($avatar)) {
	...
	function getpicarrp($rid, $from_user,$isfm = '', $goddess_provevote_picarr) {
		if ($isfm == 1) {
			$photo = pdo_fetch("SELECT photos,photoname,imgpath FROM ".tablename($goddess_provevote_picarr)." WHERE from_user = :from_user AND rid = :rid AND isfm = :isfm LIMIT 1", array(':from_user' => $from_user,':rid' => $rid,':isfm' => $isfm));
		}else {
			$photo = pdo_fetch("SELECT photos,photoname,imgpath FROM ".tablename($goddess_provevote_picarr)." WHERE from_user = :from_user AND rid = :rid ORDER BY createtime DESC LIMIT 1", array(':from_user' => $from_user,':rid' => $rid));
		}
		return $photo;
	}
