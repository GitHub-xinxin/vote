...
$op = in_array($op, array('rshare', 'rhuihua', 'rdisplay', 'rvote', 'rbody', 'rcustom', 'rupload', 'rjifen', 'raward', 'rstar')) ? $op : 'rshare';
...
if ($op == 'rshare') {
	...
	$rvote['votesuccess'] = empty($rvote['votesuccess']) ? "恭喜您成功的为编号为：#编号# ,姓名为： #参赛人名# 的参赛者投了 #投票票数# 票！":"";
...
/*	$foo = empty($_GPC['foo']) ? '' : $_GPC['foo'] ;
	...
	if ($_GPC['savegift']) {
		...
		$insert_gift = array(
			...
			'btn_font' => $_GPC['btn_font'],
			'gifturl' => $_GPC['gifturl'],
			'createtime' => time(),
		);
		if (empty($gift)) {
