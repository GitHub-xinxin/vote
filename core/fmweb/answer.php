...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		$data = array('uniacid' => $uniacid, 'rid' => $rid, 'title' => $_GPC['title'], 'answer' => iserializer($getanswers), 'thumb' => $_GPC['thumb'], 'enabled' => intval($_GPC['enabled']), 'key' => $_GPC['key'], 'displayorder' => intval($_GPC['displayorder']));
		...
		message('更新题目成功！', $this -> createWebUrl('answer', array('op' => 'display', 'rid' => $rid)), 'success');
	}
	...
	if (empty($answer)) {
		message('抱歉，题目不存在或是已经被删除！', $this -> createWebUrl('answer', array('op' => 'display', 'rid' => $rid)), 'error');
	}
	pdo_delete($this -> table_answer, array('id' => $id));
	message('题目删除成功！', $this -> createWebUrl('answer', array('op' => 'display', 'rid' => $rid)), 'success');
} else {
	message('请求方式不存在');
}
include $this -> template('web/answer');
