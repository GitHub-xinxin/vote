...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($key)) {
			message('新活动关键字不能和以前的关键字重复');
		}
		pdo_insert('rule', array('uniacid' => $uniacid, 'name' => $list['title'],'module' => 'goddess', 'status' =>'1'));
		$rid = pdo_insertid();
		if (!empty($rid)) {
			pdo_insert('rule_keyword', array('uniacid' => $uniacid,'rid' => $rid, 'content' => $_GPC['content'],'module' => 'goddess', 'type' =>'1', 'status' =>'1'));
			...
			message('复制成功',$this->createWebUrl('index'),'success');
		}
