...
defined('IN_IA') or exit('Access Denied');
		...
		$videolist = pdo_fetchall('SELECT from_user,video FROM '.tablename($this->table_users_videoarr).' WHERE rid = :rid ', array(':rid' => $rid) );
		$piclist = pdo_fetchall('SELECT from_user,photos FROM '.tablename($this->table_users_picarr).' WHERE rid = :rid ', array(':rid' => $rid) );

		$videos = array();		
		foreach ($videolist as $v => $row) {
			
			$user = $row['from_user'];
			$videos[$user] = $row['video'];
			
		}
		
		$facepics = array();
		foreach ($piclist as $p => $row) {
			
			$user = $row['from_user'];
			$facepics[$user] = $row['photos'];
		...
		//----------------------------------------------------------以下内容增加	
			...
			if ($videos[$usersss] || $row['youkuurl']) {
				$rightmedia = '<span class="rightmedia"><img src="'.FM_STATIC_MOBILE.'public/images/video.png?v=3.0" style="width:20px;"></span>';
				if ($videos[$usersss]) {
					$userlist[$key]['rightmedia_v'] = $gq . '<video id="videocon" controls width="100%" height="200" poster="'.tomedia($facepics[$usersss]).'" webkit-playsinline ><source src="'.tomedia($videos[$usersss]).'" type="video/mp4" /><p class="vjs-no-js">你的浏览器不支持该视频</a></p></video>';
				}elseif ($row['youkuurl']) {
