	...
	if($data){
		...
		if (file_put_contents($updir.$nfilename,base64_decode($data))===false) {
			...
			if (!$qiniu['isqiniu']) {
				...
				if (!empty($username['photoname'])){
					...
					if(!empty($mygift['realname'])){
						//已经报名
						$this->save_modif($from_user,$rid,'img',$lastmid);
					}
				}else{
					if ($photosarrnum >= $rvote['tpxz']) {
						...
						if(pdo_get('goddess_ly_modif',array('rid'=>$rid,'from_user'=>$from_user,'item'=>'img'))){
							$fmdata['msg'] = '上传失败，您有照片在审核中,最多上传'.$rvote['tpxz'].' 张图片';
						}
						echo json_encode($fmdata);
						exit;
					}
		...
		if ($_GPC['treg'] == 1) {
			...
			$udata = array(
					...
					'nation'=>$_GPC['nation'],//民族
					'stature'=>$_GPC['stature'],//身高
					'weight'=>$_GPC['weight'],//体重
					'signature'=>$_GPC['signature'],//签名
					'interest'=>$_GPC['interest'],//兴趣
					'unionid' => $unionid,
					'school'=>$_GPC['school'],//学校
					'area'=>$_GPC['area'],
					'sex'  => $_GPC['sex'],//性别
					'id_card'=>$_GPC['id_card'],	//	省份证
					'birth'=> strtotime($_GPC['birth']),//生日
			...
			$udata['constellation'] = $this->birthext(strtotime($_GPC['birth']));
			$udata['age'] = intval(((time()-strtotime($_GPC['birth']))/60/60/24/365));
			 ...
			 * 如果有用户id  则为修改信息
			 */
			if(!empty($_GPC['modif_id'])){
				//与数据库字段对比
				$has_info = pdo_get('goddess_provevote',array('id'=>$_GPC['modif_id']));
				$filed = ['realname','mobile','sex','birth','id_card','nation','area','school','stature','weight','signature','interest'];
				foreach ($filed as $key => $value) {
					if($has_info[$value] != $udata[$value]){
						$res_modif[] = $this->save_modif($from_user,$rid,$value,$udata[$value]);
					}
				}
				...
				if(!empty($res_modif)){
					if ($_GPC['templates'] == 'stylebase') {
						...
						$linkurl = $_W['siteroot'].'app/'.$this->createMobileUrl('reg', array('rid' => $rid,'tfrom_user' => $from_user));
					}
					$fmdata = array(
						"success" => 1,
						"msg" => '资料已提交,等待审核',
