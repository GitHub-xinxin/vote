 ...
 * 获取礼物列表
 */
$gift_list = pdo_getall($this->table_jifen_gift,array('rid'=>$rid));
 ...
 * @var [type]
 */
$is_has_area = pdo_get('goddess_voteer',array('rid'=>$rid,'from_user'=>$from_user))['area'];
$vfrom = $_GPC['do'];
if ($rvote['votepay']==1) {
...
if(!empty($tfrom_user)) {
	...
	$user['school'] = pdo_get('goddess_ly_school',array('id'=>$user['school']))['name'];
...
$picarrs =  pdo_fetchall("SELECT id, photos,from_user FROM ".tablename($this->table_users_picarr)." WHERE from_user = :from_user AND rid = :rid AND new_img = 0 ORDER BY isfm DESC ", array(':from_user' => $user['from_user'],':rid' => $rid));
