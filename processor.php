...
class goddessModuleProcessor extends WeModuleProcessor {
	public $title 			 = '女神来了！';
	...
	public $table_pnametag 	 = 'goddess_pnametag';	//默认口号
	public $table_voteer  	 = 'goddess_voteer';	//投票的人资料
	...
	public $table_qunfa        = 'goddess_qunfa';//投票记录
	public $table_shuapiao        = 'goddess_vote_shuapiao';//封禁记录
	public $table_shuapiaolog        = 'goddess_vote_shuapiaolog';//刷票记录
	...
	public $table_designer_menu   = 'goddess_templates_designer_menu';//模板页面
	public $table_order   = 'goddess_order';//付费投票
	public $table_jifen   = 'goddess_jifen';//积分抽奖
	public $table_jifen_gift   = 'goddess_jifen_gift';//礼物
	public $table_user_gift   = 'goddess_user_gift';//礼物
	public $table_user_zsgift   = 'goddess_user_zsgift';//礼物
	public $table_msg   = 'goddess_message';//消息
	public $table_orderlog   = 'goddess_orderlog';//消息
	public $table_counter   = 'goddess_counter';//投票数据统计
	public $table_qrcode   = 'goddess_qrcode';//投票二维码
	...
	public function respond() {
		...
		load()->func('communication');
			...
			if ($rbasic['status']==0){
				...
				$message .= "1、<a href='".$_W['siteroot']."app/index.php?i=".$uniacid."&c=entry&rid=".$rid."&m=goddess&do=paihang'>看看排行榜</a>\n";
				...
				if($now >= $rbasic['start_time'] && $now <= $rbasic['end_time']){
					...
					if($now <= $rbasic['start_time']){
						...
						$message .= "1、<a href='".$_W['siteroot']."app/index.php?i=".$uniacid."&c=entry&rid=".$rid."&m=goddess&do=photosvote'>先睹为快</a>\n";
						if (($rhuihua['ishuodong'] == 1 || $rhuihua['ishuodong'] == 3) && !empty($rhuihua['huodongurl'])) {
							$message .= "2、<a href='".$rhuihua['huodongurl']."'>".$rhuihua['huodongname']."</a>";
						}
	...
	public function tp($rid, $word, $from_user, $uniacid,$rvote,$rdisplay, $rhuihua,$rbasic) {
		...
		if (empty($t)) {
			...
			$message .= "<a href='".$_W['siteroot']."app/index.php?i=".$uniacid."&c=entry&rid=".$rid."&m=goddess&do=photosvote&from_user=".$from_user."'>活动首页</a>\n";
		...
		if (!$tpxz_status) {
			...
			$message .= "<a href='".$_W['siteroot']."app/index.php?i=".$uniacid."&c=entry&tfrom_user=".$tfrom_user."&rid=".$rid."&m=goddess&do=tuser'>围观Ta</a>\n";
			return $message;
		}
		...
		if (($rhuihua['ishuodong'] == 1 || $rhuihua['ishuodong'] == 3) && !empty($rhuihua['huodongurl'])) {
			...
			$rowhd['url'] = $rhuihua['huodongurl']."&from=goddess&oid=".$from_user;
			$news[] = $rowhd;
		}
		return $news;
	}
